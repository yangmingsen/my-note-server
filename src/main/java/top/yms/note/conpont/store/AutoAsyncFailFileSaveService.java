...
public class AutoAsyncFailFileSaveService implements NoteTask, InitializingBean {
    ...
    public void run() {
        while (true) {
            try {
                Object oV = cacheService.blPop(NoteCacheKey.ASYNC_UPLOAD_FILE_FAIL_LIST, 15, TimeUnit.SECONDS);
                if (oV == null) {
                    continue;
                }
                ...
                log.error("AutoAsyncFailFileSaveService error", e);
            }
        }
    }
    ...
    public void afterPropertiesSet() throws Exception {
        thread.start();
        log.info("AutoAsyncFailFileSaveService started .....");
    }
}
